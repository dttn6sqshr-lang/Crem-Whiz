...
class Game(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.guess_streaks = defaultdict(int)  # Tracks consecutive correct guesses
        self.extra_hints = defaultdict(int)    # Extra hints awarded for streaks
    ...
    async def hint(self, interaction: discord.Interaction):
        ...
        total_hints = 3 + self.extra_hints.get(user, 0)

        if self.hints_used >= total_hints:
            await interaction.response.send_message("No more hints left for this round!")
            return

        hint_text = self.next_hint()
        hints_left = total_hints - self.hints_used
        await interaction.response.send_message(
            f"💡 Hint: {hint_text}\nPoints will be reduced for using hints. Hints left this round: {hints_left}"
        )
        ...
        if guess == word_to_guess:
            ...
            if self.guess_streaks[user] > 0 and self.guess_streaks[user] % 3 == 0:
                ...
                await message.channel.send(
                    f"🔥 {user} has a {self.guess_streaks[user]}-round streak! You earned 1 extra hint for the next round."
                )
