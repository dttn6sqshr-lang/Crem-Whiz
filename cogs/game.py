...
class Game(commands.Cog):
    ...
    async def end_game(self, manual=False):
        ...
        # Purge guess channel if applicable
        if self.channel.name == GUESS_CHANNEL_NAME:
            deleted = await self.channel.purge(limit=100, check=lambda m: not m.pinned)
            await self.send_log("Channel Purged",
                                f"{self.channel.name} purged {len(deleted)} messages",
                                f"Server: {self.channel.guild.name} | Round {self.round}")
    ...
    async def guess_purger_task(self):
        ...
        if channel and not self.game_running:
            ...
            await self.send_log("Channel Purged (24h)",
